...
class BooksApp extends React.Component {
  ...
  handleStatusChange = (status, title) => {
    let keys = fetchKeyArrayfromDict(this.state.bookShelfStatus);
    if(keys.includes(status)){
      let book = this.state.books.filter(book => book.title === title);
      book[0]['shelf'] = status;
  ...
  handleSearchQuery = query => {
    if(query.length===0){
      ...
      let searchedBookByTitle = this.state.books.filter(book => 
        book['title'].toLowerCase() === query.toLowerCase());
      let searchedBookByAuthors = this.state.books.filter(book => 
        book['authors']
        .map(
          (author) => author.toLowerCase()
        )
        .includes(query.toLowerCase()));
      let searchedBookByCategories = this.state.books.filter(book => 
        book['categories']
        .map(
      ...
      this.setState(prevState => ({
        searchedBooks: [...new Set([ 
          ...searchedBookByTitle, 
          ...searchedBookByAuthors,
          ...searchedBookByCategories])],
      }));
    }
  } 
